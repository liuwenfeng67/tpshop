...
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type!=0 and price!=0 and to_days(now())-to_days(from_unixtime(paytime))=0;
declare c_user_d2 cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type!=0 and price!=0 and to_days(now())-to_days(from_unixtime(paytime))=1;
declare c_course_d cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type=0 and price!=0 and to_days(now())-to_days(from_unixtime(paytime))=0;
declare c_course_d2 cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type=0 and price!=0 and to_days(now())-to_days(from_unixtime(paytime))=1;
declare c_train_d cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_pxorder_info where status=1 and partition_id=pid and order_price!=0 and to_days(now())-to_days(order_paytime)=0;
declare c_train_d2 cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_pxorder_info where status=1 and partition_id=pid and order_price!=0 and to_days(now())-to_days(order_paytime)=1;
declare c_test_d cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_test_order where status=1 and partition_id=pid and price!=0 and to_days(now())-to_days(from_unixtime(pay_time))=0;
declare c_test_d2 cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_test_order where status=1 and partition_id=pid and price!=0 and to_days(now())-to_days(from_unixtime(pay_time))=1;
declare c_goods_d cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_shop_order where order_status=1 and partition_id=pid and order_price!=0 and to_days(now())-to_days(order_paytime)=0;
declare c_goods_d2 cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_shop_order where order_status=1 and partition_id=pid and order_price!=0 and to_days(now())-to_days(order_paytime)=1;

-- 月
declare c_user_m cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type!=0 and price!=0 and from_unixtime(paytime,'%Y-%m')=date_format(curdate(),'%Y-%m');
declare c_user_m2 cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type!=0 and price!=0 and period_diff(date_format(now(),'%Y%m'),from_unixtime(paytime,'%Y%m'))=1;
declare c_course_m cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type=0 and price!=0 and from_unixtime(paytime,'%Y-%m')=date_format(curdate(),'%Y-%m');
declare c_course_m2 cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type=0 and price!=0 and period_diff(date_format(now(),'%Y%m'),from_unixtime(paytime,'%Y%m'))=1;
declare c_train_m cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_pxorder_info where status=1 and partition_id=pid and order_price!=0 and date_format(order_paytime,'%Y-%m')=date_format(curdate(),'%Y-%m');
declare c_train_m2 cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_pxorder_info where status=1 and partition_id=pid and order_price!=0 and period_diff(date_format(now(),'%Y%m'),date_format(order_paytime,'%Y%m'))=1;
declare c_test_m cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_test_order where status=1 and partition_id=pid and price!=0 and from_unixtime(pay_time,'%Y-%m')=date_format(curdate(),'%Y-%m');
declare c_test_m2 cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_test_order where status=1 and partition_id=pid and price!=0 and period_diff(date_format(now(),'%Y%m'),from_unixtime(pay_time,'%Y%m'))=1;
declare c_goods_m cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_shop_order where order_status=1 and partition_id=pid and order_price!=0 and date_format(order_paytime,'%Y-%m')=date_format(curdate(),'%Y-%m');
declare c_goods_m2 cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_shop_order where order_status=1 and partition_id=pid and order_price!=0 and period_diff(date_format(now(),'%Y%m'),date_format(order_paytime,'%Y%m'))=1;

-- 年
declare c_user_y cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type!=0 and price!=0 and from_unixtime(paytime,'%Y')=date_format(curdate(),'%Y');
declare c_user_y2 cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type!=0 and price!=0 and period_diff(date_format(now(),'%Y'),from_unixtime(paytime,'%Y'))=1;
declare c_course_y cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type=0 and price!=0 and from_unixtime(paytime,'%Y')=date_format(curdate(),'%Y');
declare c_course_y2 cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_online_order where status=1 and partition_id=pid and type=0 and price!=0 and period_diff(date_format(now(),'%Y'),from_unixtime(paytime,'%Y'))=1;
declare c_train_y cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_pxorder_info where status=1 and partition_id=pid and order_price!=0 and date_format(order_paytime,'%Y')=date_format(curdate(),'%Y');
declare c_train_y2 cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_pxorder_info where status=1 and partition_id=pid and order_price!=0 and period_diff(date_format(now(),'%Y'),date_format(order_paytime,'%Y'))=1;
declare c_test_y cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_test_order where status=1 and partition_id=pid and price!=0 and from_unixtime(pay_time,'%Y')=date_format(curdate(),'%Y');
declare c_test_y2 cursor for
select ifnull(count(*),0),ifnull(sum(price),0) from tt_test_order where status=1 and partition_id=pid and price!=0 and period_diff(date_format(now(),'%Y'),from_unixtime(pay_time,'%Y'))=1;
declare c_goods_y cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_shop_order where order_status=1 and partition_id=pid and order_price!=0 and date_format(order_paytime,'%Y')=date_format(curdate(),'%Y');
declare c_goods_y2 cursor for
select ifnull(count(*),0),ifnull(sum(order_price),0) from tt_shop_order where order_status=1 and partition_id=pid and order_price!=0 and period_diff(date_format(now(),'%Y'),date_format(order_paytime,'%Y'))=1;
...
select ifnull(sum(price),0) into repeat_vip from tt_online_order where status=1 and partition_id=pid and type != 0 and period_diff(date_format(curdate(),'%Y%m'),from_unixtime(paytime,'%Y%m'))=i;
select ifnull(sum(price),0) into repeat_course from tt_online_order where status=1 and partition_id=pid and type=0 and period_diff(date_format(curdate(),'%Y%m'),from_unixtime(paytime,'%Y%m'))=i;
select ifnull(sum(order_price),0) into repeat_train from tt_pxorder_info where status=1 and partition_id=pid and period_diff(date_format(curdate(),'%Y%m'),date_format(order_paytime,'%Y%m'))=i;
select ifnull(sum(order_price),0) into repeat_goods from tt_shop_order where order_status=1 and partition_id=pid and period_diff(date_format(curdate(),'%Y%m'),date_format(order_paytime,'%Y%m'))=i;
select ifnull(sum(price),0) into repeat_test from tt_test_order where status=1 and partition_id=pid and period_diff(date_format(curdate(),'%Y%m'),date_format(from_unixtime(pay_time),'%Y%m'))=i;
